...
class SubmitViewModel(app: Application) : AndroidViewModel(app) {
    ...
    private val _items: MutableLiveData<Pair<State, List<RecyclerViewCell>>> =
        MutableLiveData(State.CELLS to mutableListOf())
    val items: LiveData<Pair<State, List<RecyclerViewCell>>> = _items

    fun getMItems(): LiveData<Pair<State, List<RecyclerViewCell>>> = _items
    ...
    fun getData(): List<RecyclerViewCell> {
        if ((_items.value?.second == null) or (_items.value?.second?.isEmpty()!!)) {
            initData()
        }
        return _items.value?.second!!
    }
    ...
    * */
    ...
    fun dataUpdate(position: Int, newValue: Int): Boolean {
        val pair = _items.value!!
        val item = pair.second[position]
        ...
        return when (item) {
            is ClassicRateDataClass -> {
                ...
                _items.value = pair.first to newItems
                true
            }
            is CrowdRateDataClass -> {
                newItems.apply {
    ...
    fun dataUpdate(
        ...
        val pair = _items.value!!
        val item = pair.second[position] as RateWithCheckBoxDataClass
        val newItems: MutableList<RecyclerViewCell> = mutableListOf()
        newItems.addAll(getData())
    ...
    fun dataUpdate(position: Int, newValue: String): Boolean {
        ...
        val pair = _items.value!!
        val item = pair.second.get(position) as FeedbackDataClass
        val newItems: MutableList<RecyclerViewCell> = mutableListOf()
        newItems.addAll(getData())
    ...
    fun userClickOnSubmitButton() {
        _items.value = State.PROGRESS to _items.value?.second!!
        viewModelScope.launch {
            delay(5000L)
            _submitEvent.call()
            _items.value = State.CELLS to _items.value?.second!!
        }
    }

    private fun initData() {
        _items.value = _items.value?.first!! to listOf(
            createCrowdRateCell(
